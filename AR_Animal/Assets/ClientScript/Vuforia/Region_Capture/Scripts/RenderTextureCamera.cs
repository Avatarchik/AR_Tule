...
public class RenderTextureCamera : MonoSingleton<RenderTextureCamera> {
	...
	void Start() {
        ...
        gameObject.layer = LayerMask.NameToLayer(Region_Capture.LayerName);
      // Render_Texture_Camera.cullingMask = 1 << LayerMask.NameToLayer(Region_Capture.LayerName);
        Render_Texture_Camera.cullingMask = LayerMask.GetMask(Region_Capture.LayerName);
  StartCoroutine(ShowTextureOnGUI());
	...
	private IEnumerator RecalculateRenderTexture() {
        ...
        while(Render_Texture_Camera == null)
        {
            yield return 0;
        }

        Camera cam = Render_Texture_Camera.GetComponent<Camera>();
        if (cam != null)
        {
